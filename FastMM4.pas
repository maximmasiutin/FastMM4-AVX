...
{$IFDEF EnableFSRM}
  ...
  // them.
  // The size of that startup cost is what makes the test worth its own branch.
  // Intel gives the figures for Nehalem, where rep movsb starts at 50 cycles
  // for a string longer than nine bytes, and rep movsw, rep movsd and rep movsq
  // start at 35 to 40, so at these lengths the setup dominates the copy. Later
  // parts differ, and the figures are quoted here for the size of the effect
  // rather than as constants. See https://stackoverflow.com/a/45123049/6910868
  push    rsi
  push    rdi
  call    GetFastMMCpuFeaturesA
  pop     rdi
