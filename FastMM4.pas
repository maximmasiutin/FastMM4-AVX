...
{$ENDIF}
  ...
  moreover, it can make subsequent SSE code slower. That last part holds for
  the AMD parts, which keep the two halves of a vector register apart, and not
  for Intel from Skylake on, where the save and restore stall gave way to a
  false dependency that VZEROUPPER is still the way out of. Agner Fog measured
  on Haswell that touching one ymm register puts every vector register into a
  dirty upper state, so a later non-VEX write to an xmm register takes a false
  dependency on its own previous value, see
