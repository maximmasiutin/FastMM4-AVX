...
{$ENDIF}
  ...
  What replaced the penalty differs by vendor, and neither case leaves the AVX2
  routines below needing VZEROUPPER. AMD keeps the halves of a vector register
  independent. Intel from Skylake on turned the save and restore stall into a
  false dependency instead: Agner Fog measured on Haswell that touching one ymm
  register puts every vector register into a dirty upper state, so a later
  non-VEX write to an xmm register takes a false dependency on its own previous
  value (see https://stackoverflow.com/a/43881748/6910868). The AVX2 routines
  avoid arriving in that state rather than clearing it afterwards: each one
  finishes by zeroing the registers it touched with a VEX encoded vpxor, and a
  VEX write to an xmm register zeroes bits 128 to 255 as well, so no upper half
  is left dirty for the next non-VEX instruction to depend on. The AVX1
  routines cannot do that, because they clear with vxorps, and they call
  VZEROUPPER on entry and on exit instead.}
    ...
    begin
      ...
      begin
        {VirtualQuery may fail for addresses >2GB if a large address space is
         ...
         64-bit host move that boundary
         (see https://stackoverflow.com/a/44863475/6910868).}
